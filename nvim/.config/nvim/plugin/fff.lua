...
end, "Live fffuzy grep")

map("<leader>sw", function()
	require("fff").live_grep({ query = vim.fn.expand("<cword>") })
end, "Search current word")

-- local fff_picker = require("fff-picker")
-- map("<leader>sf", fff_picker.fff, "Find files (fff)")
-- map("<leader>sF", fff_picker.live_grep, "Live grep (fff)")
-- map("<leader>sz", function() -- fuzzy-only mode
-- 	fff_picker.live_grep({ modes = { "fuzzy" } })
-- end, "Fuzzy grep (fff)")
